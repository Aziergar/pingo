...
class Canvas
{
    setup(divID, _width, _height, color = white)
    {
        ...
        this.canvas = createCanvas(_width, _height/*, WEBGL*/);
...
class Marker extends Instrument
{
    use(data = {mouse : canvas.getMouse(), pmouse : canvas.getPMouse()})
    {
        ...
        line(data.pmouse.x - 400, data.pmouse.y - 300, data.mouse.x - 400, data.mouse.y - 300);
        pop();
        this.data = data;
        return JSON.stringify(this);
    }
...
class Spray extends Instrument
{
    use(data = {mouse : canvas.getMouse(), pmouse : canvas.getPMouse(), seed : random()})
    {
        push();
        shader(sh);
        rect(data.mouse.x - 400, data.mouse.y - 300, this.thickness, this.thickness);
